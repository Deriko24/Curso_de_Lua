...
fucntion = palavra_chave; nome_da_função;(macaco = variaveis_locais)
    print (macaco)   conteudo da função

   end = determina o fim da função

        exemplo pratico ]]

function saudacao()
    print("Olá, seja bem-vindo ao Lua!")
end

saudacao()
 -- sub-Capitulo 5: if; then; else; elseif

function compara(Valor1 , Valor2)
    if Valor1 < Valor2 then -- if = ser; then = então
    print("Valor1 e menor que o Valor2")
    end
end
    compara(12, 24) -- chamar função e dar valo para elas
-- o usou geral de if, then, else

function calculadora(Valor1, Valor2)
    if Valor1 < Valor2 then
        print("Eu gosto de algo legal")
    elseif Valor1 > Valor2 then
        print("Eu não gosto disso")
    else
        print("Ser da o cu com nós ou dá para nós")
    end
end
calculadora(1, 1)

--[[ explicação dos comandos if = ser. sempre tem que ter if no inicio e no fim then = então
elseif = ser a condição de cima for falsa então veja a minha. then e sempre no final da mesma
linha.
else = ser todas as opções anterios forma falsa então me usar. then não tem na mesma linha.
 A estrutura e muito importante cuidado com isso. 
    
    Um resumo do chat lgbt
1. if:
Usado para verificar se uma condição é true. Se for, 
o bloco de código após then será executado.


2. elseif:
Usado para adicionar mais condições se a primeira condição
if for false. Você pode usar vários elseif.


3. else:
Usado quando todas as condições anteriores são false. 
O bloco de código dentro do else será executado.
    
    sub Capitulo 5: and e or
    
    Elas são usadas nas condições. tipo 
    if  conta and conta then
        
Nessa parte ele analisar ser as duas contas são verdadeira
ser as duas forem verdadeira ele realiza a função mas ser qual que
uma das duas for falsa a afimação inteira e falsa.
        
        if conta or conta then
            
Ser qual que um dois lado for verdadeiro a formula inteira e verdade.
